...
Write-Host -ForegroundColor White "|              Disabling Side Navigation                 |"
Write-Host -ForegroundColor White "--------------------------------------------------------"
...
try
{
    ...
    $customAction = Get-SPOCustomAction -Scope Site | where { $_.Name -eq "Negotium.SideNavigation" }
    Remove-SPOCustomAction -Identity $customAction.Id -Scope Site -Force

    Write-Host -ForegroundColor Green "Side Navigation removal succeded"
}
catch
{
